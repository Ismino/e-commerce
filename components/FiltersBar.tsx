      ...
      <div className="grid grid-cols-1 gap-2 rounded-2xl border border-white/70 bg-white/90 p-3 backdrop-blur md:grid-cols-4">
        ...
        <div className="grid grid-cols-3 gap-2 md:col-span-2">
          <div>
            ...
            <input id="min" inputMode="numeric" pattern="[0-9]*" className={inputCls} value={priceMin} onChange={(e) => update({ priceMin: e.target.value || undefined, page: 1 })} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-rose-700" htmlFor="max">Max price</label>
            <input id="max" inputMode="numeric" pattern="[0-9]*" className={inputCls} value={priceMax} onChange={(e) => update({ priceMax: e.target.value || undefined, page: 1 })} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-rose-700" htmlFor="rating">Min rating</label>
            <input id="rating" inputMode="decimal" placeholder="0–5" className={inputCls} value={ratingMin} onChange={(e) => update({ ratingMin: e.target.value || undefined, page: 1 })} />
          </div>
        </div>

        <div className="flex items-end justify-end md:col-span-2">
          <button className="rounded-md border border-rose-300 bg-white px-3 py-2 text-sm text-rose-700 hover:bg-rose-50" onClick={resetFilters} aria-label="Reset filters">
            Reset
          </button>
        </div>
      </div>
